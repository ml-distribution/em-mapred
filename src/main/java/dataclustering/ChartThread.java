...
public class ChartThread extends Thread {
	...
	private final Vector<Point3D> VP;
	private final int K;
	private final int MaxLoops;
	private final Plot3DPanel ClusteringPanelChart;
	private final JTable jTable;
	private final JLabel jLabel4;
	private final JButton jButton2;
	private final int KMeansType;
	private final JTabbedPane jTabbedPane;
	private final int DistanceType;

	ChartThread(int KMeansType, Vector<Point3D> VP, int K, int MaxLoops, Plot3DPanel ClusteringPanelChart,
			JTable jTable, JLabel jLabel4, JButton jButton2, JTabbedPane jTabbedPane, int DistanceType) {
	...
	private Vector<Cluster> K_Means_Clustering(Cluster MainCluster, int K, int MaxLoops, int[] Seeds, boolean showChart) {
		...
		while (loops < Math.min(10, MaxLoops)) {
			...
			for (int i = 0; i < K; i++) {
				if (!CONST.Equal(ClusterVector.get(i).Centroid, CONST.getCentroid(ClusterVector.get(i).V),
						this.DistanceType)) {
					Terminate = false;
				}
	...
	private void EM_K_Means_Clustering(Cluster MainCluster, int K) {
		...
		for (int k = 0; k < R; k++) {
			...
			for (int i = 0; i < K; i++) {
				PxBelongsToC[k][i] = CONST.NormalDistribution(MainCluster.V.elementAt(k), Means[i], Deviation)
						/ SumOfPxBelongsToC;
			}
		}
	...
	private void createKMeansChart() {
		...
		jTable.setModel(new javax.swing.table.DefaultTableModel(new Object[K][5], new String[] { "Cluster No",
				"NOf Objects", "Centroid's X", "Centroid's Y", "Centroid's Z" }) {

			private static final long serialVersionUID = -7655622662931337239L;
