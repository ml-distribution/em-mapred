...
public class ExpMax {
	...
	private final double probEpsilon = 0.05;
	...
	public void calculateParameters() {
		...
		do {
			...
			iterations++;
		} while (means.size() != oldMeans.size() || arrayListDifferent(means, oldMeans)
				|| matrixListDifferent(covs, oldCovs));
		System.out.println("EM on " + name + " data complete! " + means.size() + " clusters found. Took " + iterations
				+ " iterations and " + Double.toString((System.currentTimeMillis() - startTime) / 1000.0) + " seconds");
		printParameters();
	}
	...
	public String compareToSeed() throws IOException {
		try {
			...
			double[][] seeds = Util.importFile(Util.getFilepath("data", locationType, name, ".clusters"));
			for (double[] seed : seeds) {
				...
				for (int i = 0; i < means.size(); i++) {
					...
					System.err.println(Arrays.toString(seed) + " " + Arrays.toString(means.get(i)) + " " + dist);
					if (dist < min) {
						min = dist;
						closestCluster = i;
					}
				...
				comparisonInfo += (Arrays.toString(seed) + " is closest to "
						+ Arrays.toString(means.get(closestCluster)) + " at a distance of " + min + ". This is " + (min < 5 ? "close."
						: "not close."))
						+ "\n";
				if (min < 5) {
			...
			comparisonInfo += ("Matched " + clustersCloseToSeeds + " seeds to clusters out of " + seeds.length
		...
		while (currDist < means.size()) {
			...
			for (int i = 0; i < numPoints; i++) {
				...
				for (int j = 0; j < currDist; j++) {
					double distSq = Util.distanceSquared(currPoint, means.get(j));
					if (distSq < minDistSq)
						minDistSq = distSq;
				}
				weights[i] = minDistSq;
	...
	private void initializeCovs() {
		...
		for (int i = 0; i < minClusters; i++) {
			...
			for (int r = 0; r < dim; r++)
				for (int c = 0; c < dim; c++)
					if (r == c)
						covs.get(i).setEntry(r, c, covInit);
		}
	}
			...
			for (int i = 0; i < means.size(); i++) {
				...
				for (int j = 0; j < newMeans.size(); j++) {
					...
					if (areDuplicates(i, j)) {
						System.err.println("DUPLICATE DIST: " + Arrays.toString(oldMean));
	 ...
	 */
	private boolean arrayListDifferent(ArrayList<Double[]> curr, ArrayList<Double[]> old) {
	 ...
	 */
	private boolean matrixListDifferent(ArrayList<RealMatrix> curr, ArrayList<RealMatrix> old) {
		for (int i = 0; i < curr.size(); i++)
		...
		while (currDist < means.size()) {
			...
			try {
				if (Util.isValidCovarianceMatrix(covs.get(currDist))) {
					dists.add(new MultivariateNormalDistribution(Util.doubleValues(means.get(currDist)), covs.get(
							currDist).getData()));
					currDist++;
					i = 1;
				} else {
					replaceCluster(currDist, covInit * i++);
					System.err.println("REPLACING DIST: " + i + " " + Arrays.toString(means.get(currDist))
							+ " invalid cov");
				}
			} catch (MaxCountExceededException ex) {
				replaceCluster(currDist, covInit * i++);
				System.err.println("REPLACING DIST: " + Arrays.toString(means.get(currDist)) + " cov error");
			}
		}
	}
		...
		if (expectedValues.size() < maxClusters) {
			for (int i = 0; i < numPoints; i++) {
				...
				if (distributionUniformJS(dist)) {
					...
					dists.add(new MultivariateNormalDistribution(Util.doubleValues(means.get(means.size() - 1)), covs
							.get(covs.size() - 1).getData()));
					expectedValues.add(new Double[numPoints]);
					for (int c = 0; c < means.size(); c++)
						for (int k = 0; k < numPoints; k++)
							expectedValues.get(c)[k] = expectedValuePoint(getPoint(k), c);
		...
		for (int i = 0; i < means.size(); i++) {
			...
			for (int r = 0; r < dim; r++) {
				for (int c = 0; c < dim; c++) {
					double entry = 0;
					for (int j = 0; j < numPoints; j++) {
						entry += ((expectedValues.get(i)[j]) * (data[r][j] - means.get(i)[r]) * (data[c][j] - means
								.get(i)[c]));
					}
			...
			if (covarianceTooLarge(i)) {
				...
				System.err.println("REPLACING DIST: " + Arrays.toString(means.get(i)) + " cov too large");
			}
			if (!Util.isValidCovarianceMatrix(covs.get(i))) {
				replaceCluster(i, covInit);
				System.err.println("REPLACING DIST: " + Arrays.toString(means.get(i)) + " invalid cov");
			}
		}
	}
}