...
public class ExpMaxTest {
	...
	private void testRun(int numDist, int numPoints) {
		...
		for (int i = 0; i < numDist; i++) {
			...
			for (int r = 0; r < dim; r++)
				for (int c = 0; c < dim; c++)
					if (r == c)
						cov[r][c] = 5.0;
			MultivariateNormalDistribution dist = new MultivariateNormalDistribution(means[i], cov);
		...
		System.out.println("\nEm with k=" + numDist + " and n=" + numPoints);
	...
	public void testData(String locationType, String locationName, int i, int j) throws IOException {
		//		String filepath = File.separator + locationType + File.separator + locationName;
		double[][] data = Util.importFile(Util.getFilepath("data", locationType, locationName, ".data"));
		ExpMax em = new ExpMax(data, i, j, locationName, locationType);
		em.calculateParameters();
		Util.writeFile(Util.getFilepath("results", locationType, locationName, ".comp"), em.compareToSeed());
		em.exportParameters(Util.getFilepath("results", locationType, locationName, ".results"));
	}
	...
	public void testDataNoMin(String locationType, String locationName) throws IOException {
		...
		em.calculateParameters();
		Util.writeFile(Util.getResultsFilepath("nomin", locationType, locationName, ".comp"), em.compareToSeed());
		em.exportParameters(Util.getResultsFilepath("nomin", locationType, locationName, ".results"));
	}
	...
	public void testDataFixed(String locationType, String locationName, int i) throws IOException {
		...
		em.calculateParameters();
		Util.writeFile(Util.getResultsFilepath("fixed", locationType, locationName, ".comp"), em.compareToSeed());
		em.exportParameters(Util.getResultsFilepath("fixed", locationType, locationName, ".results"));
	}

	@Test
	public void testNoMin() throws IOException {
		String[] cities = new String[] { "Yorkshire_(disambiguation)", "Bermuda_(disambiguation)", "Newcastle",
				"Aberdeen_(disambiguation)", "Camden", "Erie_(disambiguation)" };
		String[] counties = new String[] { "Lake_County", "Marion_County", "Montgomery_County", "Monroe_County",
				"Carroll_County", "Grant_County" };
		for (String c : cities) {
