...
public class BaumWelch {
	...
	public static HiddenMarkovModel infer(String s, int numStates, double pChange) {
	...
	public BaumWelch(String s, int numStates) {
		...
		for (int i = 0; i < N; i++) {
			...
			for (int j = 0; j < N; j++) {
				states[i].addTransitionProbability(String.valueOf(j), transition[j]);
			}
		}
	...
	public double maximize() {
		...
		for (int i = 0; i < N; i++) {
			alpha[i][0] = Math.log(states[i].getStartProbability())
					+ Math.log(states[i].getEmissionProbability(observations.charAt(0)));
		}
		...
		for (int t = T - 2; t >= 0; t--) {
			for (int i = 0; i < N; i++) {
				...
				for (int k = 0; k < N; k++) {
					sumBeta[k] = beta[k][t + 1] + Math.log(states[i].getTransitionProbability(states[k].getName()))
							+ Math.log(states[k].getEmissionProbability(observations.charAt(t + 1)));
				}
		...
		for (int t = 0; t < T - 1; t++) {
			for (int i = 0; i < N; i++) {
				for (int j = 0; j < N; j++) {
					xiNum[i][j][t] = alpha[i][t] + Math.log(states[i].getTransitionProbability(states[j].getName()))
							+ Math.log(states[j].getEmissionProbability(observations.charAt(t + 1))) + beta[j][t + 1];
		...
		for (int i = 0; i < N; i++) {
			for (int k = 0; k < alphabet.length; k++) {
				...
				if (p == 0) {
					...
					newEmissionProb[i][k] = Math.exp(logsum(numExp, p - 1) - sumGammaoverTT[i]);
			}
		}
