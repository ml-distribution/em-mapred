...
public class EM {
	...
	private final int numExperiments, numTypes, numResults;
	...
	public EM(int numTypes, int[][] counts) {
		this(numTypes, counts, getRandomProbabilities(numTypes, counts[0].length), getRandomProbabilities(numTypes));
	}
	 ...
	 */
	public EM(int numTypes, int[][] counts, double[][] startResultProb, double[] startTypeProb) {
		...
		for (int i = 0; i < numExperiments; i++) {
			for (int j = 0; j < numTypes; j++) {
				for (int k = 0; k < numResults; k++) {
					expected[i][j][k] = Math.log(posterior[i][j]) + Math.log(counts[i][k]);
				}
			}
		}
	...
	public double maximizeWeights(double[][] expectedTotal) {
		...
		for (int a = 0; a < numTypes; a++) {
			for (int b = 0; b < numResults; b++) {
				diff += Math.abs(newPrior[a][b] - resultProb[a][b]);
			}
		}
	 ...
	 */
	private static double[][] getRandomProbabilities(int numTypes, int numResults) {
		Random r = new Random();
